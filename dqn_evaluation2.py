...
if __name__ == '__main__':
    ...
    wait_list = []
    for i in range(150):
        filename  = "./params/%s/net_params_%d.pkl"%(cmd_args.folder_name, i)
    ...
    filename = "./params/%s/net_params_%d.pkl" % (cmd_args.folder_name, best_network_index)
    target_net = DQN(screen_height, screen_width, n_actions).to(device)
    target_net.load_state_dict(torch.load(filename))
    wait_total = 0
    for i in range(5):
    ...
    print('delay_time %d average waiting time %f\n' % (cmd_args.delay_time, wait_average))